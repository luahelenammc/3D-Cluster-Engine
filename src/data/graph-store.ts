...
export class GraphStore {
  ...
  private commit(next: GraphDataset) {
    ...
    if (!result.valid) throw new Error(result.issues.filter((x) => x.severity === "error").map((x) => x.message).join("; "));
  ...
  addLink(link: GraphLink) { this.commit({ ...this.dataset, links: [...this.dataset.links, link] }); }
  updateLink(id: string, patch: Partial<GraphLink>) { this.commit({ ...this.dataset, links: this.dataset.links.map((link, index) => (link.id || `link-${index + 1}`) === id ? { ...link, ...patch } : link) }); }
  removeLink(id: string) { this.commit({ ...this.dataset, links: this.dataset.links.filter((link, index) => (link.id || `link-${index + 1}`) !== id) }); }
